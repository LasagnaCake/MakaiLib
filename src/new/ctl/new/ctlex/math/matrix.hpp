...
public:
	...
	requires Type::Matrix::ValidTransform3D<R, C> {
		compose(trans);
	}

	constexpr Matrix(Transform2D const& trans)
	requires Type::Matrix::ValidTransform2D<R, C> {
	...
	constexpr Matrix<4, 4, T> translated(Vector3 const& vec) const requires Type::Matrix::ValidTransform3D<R, C> {
	...
	constexpr Matrix<4, 4, T>& translate(Vector3 const& vec) requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		Vector4 calc =
	...
	constexpr Matrix<4, 4, T> rotated(Vector3 const& vec) const requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		return (*this) * EULER_FUNC(vec);
	}

	template<EulerFunction EULER_FUNC = Matrix::fromEulerYXZ>
	constexpr Matrix<4, 4, T>& rotate(Vector3 const& vec) requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		(*this) *= EULER_FUNC(vec);
		return (*this);
	}
	...
	// https://github.com/g-truc/glm/blob/master/glm/ext/matrix_transform.inl
	constexpr Matrix<4, 4, T> scaled(Vector3 const& vec) const requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		Vector4 result[4];
	...
	// https://github.com/g-truc/glm/blob/master/glm/ext/matrix_transform.inl
	constexpr Matrix<4, 4, T>& scale(Vector3 const& vec) requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		*this = scaled(vec);
		return (*this);
	}
	...
	constexpr Matrix<R, C, T> operator*(Transform3D const& trans) const requires Type::Matrix::ValidTransform3D<R, C>  {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
	...
	constexpr Matrix<R, C, T>& transformed(
		...
		Vector3 const& scale
	) const requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
		// Transform
	...
	const requires Type::Matrix::ValidTransform2D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
	...
	template<EulerFunction EULER_FUNC = Matrix::fromEulerYXZ>
	constexpr Matrix<R, C, T>& compose(Transform3D const& trans) requires Type::Matrix::ValidTransform3D<R, C> {
		static_assert(R == 4, "Matrix is not a valid representation of a 3D transform!");
	...
	[[gnu::unavailable("Not working as intended!")]]
	constexpr Transform3D decompose() const requires Type::Matrix::ValidTransform3D<R, C> {
		Vector4 _p;
		Vector3 _s;
