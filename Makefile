...
debug: build\$(target)
	g++.exe -fexpensive-optimizations -flto -Wall -DDEBUG -m64 -std=gnu++20 -fcoroutines -fopenmp -openmp -pg -Og -g -fsanitize=leak -fno-omit-frame-pointer -D_DEBUG_OUTPUT_ -Ilib -Ilib\SDL2-2.0.10\include -Ilib\OpenGL -Ilib\OpenGL\GLEW\include -Ilib\cute_headers -Ilib\stb -c build\$(target) -o obj\Debug\$(name).o
